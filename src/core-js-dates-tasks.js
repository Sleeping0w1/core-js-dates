...
function getNextFriday(date) {
  const weekdays = ['Thu', 'Wed', 'Tue', 'Mon', 'Sun', 'Sat', 'Fri'];
  const daysAddCount =
    weekdays.indexOf(date.toLocaleDateString('en-EN', { weekday: 'short' })) +
    1;
  date.setDate(date.getDate() + daysAddCount);
  return date;
}
...
function getNextFridayThe13th(date) {
  const result = date;
  result.setDate(13);
  while (result.toLocaleDateString('en-EN', { weekday: 'short' }) !== 'Fri') {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const curr = new Date(Date.UTC(start[2], start[1] - 1, start[0]));
  const periodEnd = new Date(Date.UTC(end[2], end[1] - 1, end[0]));
  const result = [];

  while (curr <= periodEnd) {
    for (
      let index = 0;
      index < countWorkDays && curr <= periodEnd;
      index += 1
    ) {
      const currISOS = curr.toISOString().split('T')[0];
      result.push(currISOS.split('-').reverse().join('-'));
      curr.setDate(curr.getDate() + 1);
    }
    curr.setDate(curr.getDate() + countOffDays);
  }
  return result;
}
